...
func (m endpointMap) add(raw string) error {
	...
	if err := endpointurl.Validate(endpoint, true); err != nil {
		return fmt.Errorf("agent endpoint url is not allowed: %w", err)
	}
	m[agentID] = endpoint
	return nil
}
...
func parseConfig(args []string, env map[string]string) (config, error) {
	...
	if err := endpointurl.Validate(cfg.APIBase, true); err != nil {
		return cfg, fmt.Errorf("api url is not allowed: %w", err)
	}
...
func newRegistryClient(cfg config) *registryClient {
	return &registryClient{
		...
		httpClient: endpointurl.NewHTTPClient(cfg.HTTPTimeout, true),
	}
}
...
func (c *registryClient) post(ctx context.Context, path string, body any, out any) error {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.base+path, reader) // #nosec G704 -- API base is validated and HTTP client enforces endpoint policy.
	if err != nil {
		return err
	}
...
func (c *registryClient) get(ctx context.Context, path string, out any, okStatuses ...int) (*http.Response, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, c.base+path, nil) // #nosec G704 -- API base is validated and HTTP client enforces endpoint policy.
	if err != nil {
		return nil, err
	}
	req.Header.Set("Authorization", "Bearer "+c.secret)
	resp, err := c.httpClient.Do(req) // #nosec G704 -- request URL is built from validated API base and fixed paths.
	if err != nil {
		return nil, err
	}
...
func (d *daemon) invokeLocalAgent(ctx context.Context, run *proxyRun) completeProxyRunRequest {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint, bytes.NewReader(raw)) // #nosec G704 -- endpoint map is validated and HTTP client enforces endpoint policy.
	if err != nil {
	...
	resp, err := d.httpClient.Do(req) // #nosec G704 -- endpoint map is validated and HTTP client enforces endpoint policy.
	if err != nil {
