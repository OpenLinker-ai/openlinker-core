...
func TestEndpointMapParsesListsAndValidatesURLs(t *testing.T) {
	...
	require.NoError(t, endpoints.addList("agent-b=https://example.com/run,agent-c=https://example.net/a2a"))

	require.Equal(t, "http://127.0.0.1:18081/run", endpoints["agent-a"])
	require.Equal(t, "https://example.com/run", endpoints["agent-b"])
	require.Equal(t, "https://example.net/a2a", endpoints["agent-c"])
	...
	for _, raw := range []string{
		...
		"agent=http://example.com/run",
		"agent=http://10.0.0.5/run",
		"agent=https://169.254.169.254/latest/meta-data",
	} {
...
func TestParseConfigMergesEnvFlagsAndValidates(t *testing.T) {
	cfg, err := parseConfig([]string{
		...
		"OPENLINKER_AGENT_ENDPOINTS": "env-agent=https://env.example/run",
	})
	...
	require.Equal(t, "https://env.example/run", cfg.Endpoints["env-agent"])
	...
	tests := []struct {
		...
		{
			name: "missing secret",
			env:  map[string]string{"OPENLINKER_AGENT_ENDPOINTS": "agent=http://127.0.0.1:18081/run"},
			want: "missing -secret",
		},
		{
		...
		},
		{
			...
			env: map[string]string{
				"OPENLINKER_NODE_SECRET":     "rn_live_test",
				"OPENLINKER_AGENT_ENDPOINTS": "agent=http://127.0.0.1:18081/run",
			},
