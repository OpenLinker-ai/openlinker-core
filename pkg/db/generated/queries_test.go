...
func TestUserQueriesScanRowsAndUseExpectedArgs(t *testing.T) {
	...
	adminRows := &fakeRows{rows: [][]any{adminUserRow(userRow(userID, now, &passwordHash, &provider, &oauthID, &avatar, nil), now)}}
	...
	if !adminRows.closed || len(adminUsers) != 1 || adminUsers[0].ID != userID || adminUsers[0].TaskCount != 6 {
...
func TestAgentQueriesScanRowsAndAffectedRows(t *testing.T) {
	...
	adminAgentRows := &fakeRows{rows: [][]any{adminAgentRow(agentValues, "creator@example.com", "Creator Name", now)}}
...
func TestAgentTokenQueriesScanRowsAndAffectedRows(t *testing.T) {
	...
	if !reflect.DeepEqual(dbtx.queryRowArgs, []any{tokenID, agentID, []string{"agent:call"}, creatorID, "fast-hash"}) {
		t.Fatalf("RedeemPendingAgentToken args = %#v", dbtx.queryRowArgs)
	}
...
func adminUserRow(values []any, now time.Time) []any {
