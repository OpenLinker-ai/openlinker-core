...
func TestOAuthStartAndCallbackRedirectWithCodeProviderAndCallbackURL(t *testing.T) {
	...
	startReq := httptest.NewRequest(
		...
		"/api/v1/auth/google?"+
			url.Values{
				"callbackUrl":    []string{"/runs?tab=mine"},
				"frontendOrigin": []string{"https://stage.example.test"},
			}.Encode(),
		nil,
	)
	...
	redirectState := redirectStateFromOAuthState(state, h.cfg)
	if startURL.Scheme != "https" || startURL.Host != "accounts.example" ||
		redirectState.callbackPath != "/runs?tab=mine" ||
		redirectState.frontendOrigin != "https://stage.example.test" {
	...
	if callbackURL.Scheme != "https" || callbackURL.Host != "stage.example.test" || callbackURL.Path != "/auth/callback" {
...
func TestOAuthFrontendOriginAllowlist(t *testing.T) {
	productionCfg := &config.Config{
		Env:                         "production",
		FrontendURL:                 "https://openlinker.ai",
		OAuthAllowedFrontendOrigins: "https://*.openlinker.ai, https://preview.example.test",
	}
	for _, tc := range []struct {
		name string
		raw  string
		...
		{name: "frontend url", raw: "https://openlinker.ai", want: "https://openlinker.ai"},
		{name: "wildcard subdomain", raw: "https://test.openlinker.ai", want: "https://test.openlinker.ai"},
		{name: "explicit extra", raw: "https://preview.example.test", want: "https://preview.example.test"},
		{name: "apex not matched by wildcard but matched by frontend", raw: "https://openlinker.ai/", want: "https://openlinker.ai"},
		{name: "http rejected in production", raw: "http://test.openlinker.ai", want: ""},
		{name: "foreign origin rejected", raw: "https://evil.example", want: ""},
		{name: "path rejected", raw: "https://test.openlinker.ai/path", want: ""},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if got := safeOAuthFrontendOrigin(tc.raw, productionCfg); got != tc.want {
				t.Fatalf("safeOAuthFrontendOrigin(%q) = %q, want %q", tc.raw, got, tc.want)
			}
		})
	}

	devCfg := &config.Config{Env: "development", FrontendURL: "http://localhost:3000"}
	if got := safeOAuthFrontendOrigin("http://localhost:3000", devCfg); got != "http://localhost:3000" {
		t.Fatalf("dev local origin = %q", got)
	}
}
