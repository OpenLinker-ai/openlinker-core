...
func (b *BenchmarkService) RuntimeStatus() BenchmarkRuntimeStatus {
	...
	msg := "Benchmark runtime is not ready."
	for _, r := range reasons {
		if r == "llm_not_configured" {
			msg = "Benchmark 需要 LLM 支持。自托管用户请配置 LLM_OPENAI_URL + LLM_OPENAI_API_KEY，或联系 openlinker.ai 获取 LLM 代理地址。"
		}
	}
	return BenchmarkRuntimeStatus{
		CanRun:  false,
...
func (b *BenchmarkService) RunBenchmark(ctx context.Context, agentID, creatorID uuid.UUID, skillID string) (*RunBenchmarkResponse, error) {
	if b.runner == nil || b.llm == nil {
		if b.llm == nil {
			return nil, httpx.ServiceUnavailable("benchmark 服务暂不可用：未配置 LLM。自托管请配置 LLM_OPENAI_URL + LLM_OPENAI_API_KEY")
		}
		return nil, httpx.ServiceUnavailable("benchmark 服务暂不可用： endpoint runner 未就绪")
	}
	...
	if _, err := b.q.UpsertAgentSkillScore(ctx, db.UpsertAgentSkillScoreParams{
		...
		TotalCount: clampIntToInt32(len(cases)),
	}); err != nil {
	...
	batchID := uuid.New()
	go b.runBatch(agent, skillID, batchID, cases, key) // #nosec G118 -- benchmark batch intentionally continues after request completion.
...
func (b *BenchmarkService) aggregate(ctx context.Context, agentID uuid.UUID, skillID string, batchID uuid.UUID, scores []int32, totalCases int) {
	...
	if len(scores) > 0 {
		...
		avg := clampInt64ToInt32(sum / int64(len(scores)))
		avgPtr = &avg
	...
	if _, err := b.q.UpsertAgentSkillScore(ctx, db.UpsertAgentSkillScoreParams{
		...
		AverageScore: avgPtr,
		PassCount:    passCount,
		TotalCount:   clampIntToInt32(totalCases),
		LastBatchID:  &bid,
...
func parseJudgeResponse(raw string) (int32, string, error) {
	...
	if err := json.Unmarshal([]byte(raw), &parsed); err == nil {
		return clampScore(clampFloat64ToInt32(parsed.Score)), parsed.Reason, nil
	}
	...
	score, ok := checkedInt64ToInt32(n)
	if !ok {
		return 0, "", errors.New("score out of range")
	}
	return score, truncateText(raw, 200), nil
}
