...
func TestAgentHandlersValidateBeforeServiceDispatch(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "create negative price validation", method: h.CreateAgent, req: &handlerRequest{method: http.MethodPost, target: "/", body: `{"slug":"valid-agent","name":"Valid Agent","price_per_call_cents":-1,"tags":["ai"]}`, userID: userID}, want: http.StatusUnprocessableEntity},
		...
		{name: "update negative price validation", method: h.UpdateAgent, req: &handlerRequest{method: http.MethodPatch, target: "/" + agentID, body: `{"name":"Valid Agent","price_per_call_cents":-1,"tags":["ai"]}`, userID: userID, params: map[string]string{"id": agentID}}, want: http.StatusUnprocessableEntity},
