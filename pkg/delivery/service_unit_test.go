...
func TestCreateTargetRejectsNonPublicWebhookURLsBeforeDB(t *testing.T) {
	userID := uuid.New()
	for _, rawURL := range []string{
		...
		t.Run(rawURL, func(t *testing.T) {
			...
			_, err := (&Service{queries: q, httpClient: http.DefaultClient}).CreateTarget(context.Background(), userID, &CreateTargetRequest{
				Name: "Webhook",
				Type: targetTypeWebhook,
				URL:  rawURL,
			})
			requireDeliveryHTTPStatus(t, err, http.StatusBadRequest)
			if q.createTargetArg.UserID != uuid.Nil {
				t.Fatalf("CreateTarget should reject URL before DB access, arg=%#v", q.createTargetArg)
			}
		})
	}
}
