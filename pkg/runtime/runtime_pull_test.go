...
func insertFastRuntimeToken(t *testing.T, pool *pgxpool.Pool, agentID, creatorID uuid.UUID, scopes []string) string {
	t.Helper()
	raw := make([]byte, 32)
	_, err := rand.Read(raw)
	require.NoError(t, err)
	...
	_, err = pool.Exec(context.Background(),
		`INSERT INTO agent_tokens (
			agent_id, creator_user_id, name, prefix, token_hash, scopes, status, redeemed_at
		) VALUES ($1, $2, 'test-fast-runtime', $3, $4, $5, 'active_runtime', NOW())`,
		agentID,
		creatorID,
		plaintext[:12],
		credential.FastTokenHash(plaintext),
		scopes,
	)
	require.NoError(t, err)
	return plaintext
...
func TestRuntimePull_ClaimAndCompleteSuccess(t *testing.T) {
	...
	assert.Equal(t, 5, claimed.Metadata["empty_claim_retry_after_seconds"])
...
func TestRuntimePull_FastRuntimeTokenHashCanClaim(t *testing.T) {
	...
	agentID := insertAgent(t, pool, creatorID, "https://example.com/not-used", 10, "approved")
	setRuntimePullMode(t, pool, agentID)
	token := insertFastRuntimeToken(t, pool, agentID, creatorID, []string{"agent:call", "agent:pull"})
	markRuntimePullAvailable(t, svc, token)

	started, err := svc.Run(ctx, userID, makeRunReq(agentID, map[string]any{"q": "fast hash"}), "")
	require.NoError(t, err)
...
func TestRuntimePull_ConcurrentRuntimeEventsAllocateUniqueSequences(t *testing.T) {
	pool := setupTestDB(t)
	svc := newTestService(t, pool)
	ctx := context.Background()
	...
	started, err := svc.StartRun(ctx, userID, makeRunReq(agentID, map[string]any{"q": "concurrent events"}), "api")
	require.NoError(t, err)
	runID := mustParseUUID(t, started.RunID)
	claimed, err := svc.ClaimRuntimePullRun(ctx, token)
	require.NoError(t, err)
	...
	for i := 0; i < eventCount; i++ {
		...
		go func() {
			defer wg.Done()
			_, err := svc.ReportRuntimeTokenRunEvent(ctx, token, runID, &runtime.ReportRunEventRequest{
				EventType: "run.message.delta",
				Payload: map[string]interface{}{
					"text":  "concurrent progress",
					"index": i,
				},
			})
			if err != nil {
	...
	var previous int32
	for _, event := range events {
		require.False(t, seen[event.Sequence], "duplicate sequence %d", event.Sequence)
		seen[event.Sequence] = true
		require.Greater(t, event.Sequence, previous)
		previous = event.Sequence
		if event.EventType == "run.message.delta" {
			deltaCount++
		}
	}
	require.Equal(t, eventCount, deltaCount)
}

func TestRuntimePull_ClaimIncludesCoreConversationHistory(t *testing.T) {
	pool := setupTestDB(t)
	svc := newTestService(t, pool)
	ctx := context.Background()
	...
	firstCreatorID := insertCreator(t, pool)
	firstEndpoint := startMockEndpointForService(t, svc, mockEndpointReturning(http.StatusOK, `{
		"output":{"summary":"first final"},
		"events":[{"event_type":"run.message.delta","payload":{"text":"first answer"}}]
	}`))
	...
	firstResp, err := svc.Run(ctx, userID, &runtime.RunRequest{
		...
		A2AContext: &runtime.RunA2AContextRequest{
			...
			RootContextID:     "conv-runtime-pull",
		},
	}, "api")
	require.NoError(t, err)
	require.Equal(t, "success", firstResp.Status)

	secondCreatorID := insertCreator(t, pool)
	secondAgentID := insertAgent(t, pool, secondCreatorID, "https://runtime.invalid/run", 10, "approved")
	setRuntimePullMode(t, pool, secondAgentID)
	token := insertRuntimeToken(t, pool, secondAgentID, secondCreatorID, []string{"agent:call", "agent:pull"})
	markRuntimePullAvailable(t, svc, token)
	...
	require.NoError(t, err)
	require.NotNil(t, claimed)
	require.Equal(t, started.RunID, claimed.RunID)
	require.NotNil(t, claimed.Conversation)
	assert.Equal(t, "conv-runtime-pull", claimed.Conversation.ID)
	assert.Equal(t, "conv-runtime-pull", claimed.Conversation.SessionKey)
	assert.Equal(t, started.RunID, claimed.Conversation.CurrentRunID)
	assert.Equal(t, "turn-2", claimed.Conversation.CurrentProtocolTask)
	require.Len(t, claimed.Conversation.HistoryBeforeCurrent, 2)
	assert.Equal(t, "user", claimed.Conversation.HistoryBeforeCurrent[0].Role)
	assert.Equal(t, "first question", claimed.Conversation.HistoryBeforeCurrent[0].Content)
	assert.Equal(t, "agent", claimed.Conversation.HistoryBeforeCurrent[1].Role)
	assert.Equal(t, "first answer", claimed.Conversation.HistoryBeforeCurrent[1].Content)
}

func TestRuntimePull_CompleteSuccessRecordsFreeRun(t *testing.T) {
	pool := setupTestDB(t)
	svc := newTestService(t, pool)
	ctx := context.Background()
	...
	require.NoError(t, err)
	require.Equal(t, "running", started.Status)
	require.Equal(t, int32(0), started.CostCents)
...
func TestRuntimePull_CompleteFailureRecordsFreeRun(t *testing.T) {
...
func TestRuntimePull_HeartbeatReturnsClaimHints(t *testing.T) {
	...
	assert.Equal(t, int32(5), empty.NextClaimAfterSeconds)
...
func TestRuntimePull_TimeoutNoopsAndTimesOutStaleRuns(t *testing.T) {
	pool := setupTestDB(t)
	svc := newTestService(t, pool)
	ctx := context.Background()
